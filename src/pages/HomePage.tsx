...
import {
  Search, DateRange, LocationOn, VerifiedUser,
  ArrowForward, CheckCircle,
  Bolt, WorkspacePremium, Spa, Storefront, Cancel, EmojiEvents,
  TrendingUp, Devices, Star
...
const Counter = ({ target, duration=1800 }: { target:string; duration?:number }) => {
  ...
  useEffect(() => {
    if(!active) return;
    let frame=0; const total=60; const inc=num/total;
    const t = setInterval(() => {
      frame++;
      if (frame>=total){setCount(num);clearInterval(t);}
...
const features = [
  {
    title:"Tìm sân dễ dàng", tag:"DISCOVER", num:"01",
...
const HomePage = () => {
  ...
  return (
    <Box sx={{overflow:"hidden", bgcolor:"#FFFFFF", color:"#0F172A", fontFamily:'"DM Sans",sans-serif'}}>
        ...
        <Box sx={{display:"flex", width:"max-content", animation:`${marquee} 22s linear infinite`}}>
          {[...tickerItems,...tickerItems].map((t,i)=>(
            <Box key={i} sx={{display:"flex",alignItems:"center",gap:3,px:4}}>
              <Typography sx={{fontWeight:900,fontSize:".85rem",color:"#fff",whiteSpace:"nowrap",letterSpacing:2}}>
                {t}
              </Typography>
      ...
      {/* HERO */}
      <Box sx={{
        position:"relative", minHeight:{xs:"auto", md:"90vh"},
        display:"flex", alignItems:"center",
        overflow:"hidden",
        pt:{xs:8,md:0}, pb:{xs:10,md:0},
        bgcolor: "#F9FAFB"
      }}>
        {/* Dot grid */}
        <Box sx={{position:"absolute",inset:0,zIndex:0,opacity:.4,backgroundImage:`radial-gradient(circle,#CBD5E1 1px,transparent 1px)`,backgroundSize:"28px 28px"}}/>
        
        {/* Ghost letters */}
        <Typography sx={{position:"absolute",right:{xs:"-10%",md:"-3%"},top:"50%",transform:"translateY(-50%)",fontWeight:900,fontSize:{xs:"18rem",md:"30rem"},lineHeight:1,color:"rgba(21,128,61,.03)",userSelect:"none",zIndex:0,letterSpacing:-8}}>
          PB
        </Typography>
        ...
        <Container maxWidth="lg" sx={{position:"relative",zIndex:1}}>
          <Grid container spacing={4} alignItems="center">
            ...
            <Grid item xs={12} md={7}>
              <FadeUp delay={0}>
                <Box sx={{display:"inline-flex",alignItems:"center",gap:1.5,mb:4,px:2.5,py:1,border:"1.5px solid #BBF7D0",borderRadius:10,bgcolor:"#F0FDF4"}}>
              ...
              <FadeUp delay={0.1}>
                ...
                <Typography sx={{fontWeight:900,fontSize:{xs:"2.4rem",sm:"3rem",md:"3.8rem",lg:"4.5rem"},lineHeight:1,letterSpacing:"-.01em",textTransform:"uppercase",color:"#94A3B8",mb:5}}>
                  Đặt sân dễ dàng
                </Typography>
              </FadeUp>
              ...
              <FadeUp delay={0.32}>
                <Stack direction={{xs:"column",sm:"row"}} spacing={2} sx={{mb:8}}>
                  <Button component={Link} to="/marketplace" size="large" startIcon={<Search/>} endIcon={<ArrowForward sx={{fontSize:17}}/>}
                    sx={{px:5,py:2,bgcolor:"#16A34A",color:"#fff",fontWeight:900,fontSize:"1rem",letterSpacing:1.5,textTransform:"uppercase",borderRadius:1.5,boxShadow:"0 8px 32px rgba(22,163,74,.3)",transition:"all .25s","&:hover":{bgcolor:"#15803D",transform:"translateY(-3px)",boxShadow:"0 16px 40px rgba(22,163,74,.4)"}}}>
                    Khám phá ngay
                  </Button>
                  <Button component={Link} to="/register-owner" size="large"
                    sx={{px:5,py:2,border:"2px solid #E2E8F0",color:"#334155",fontWeight:900,fontSize:"1rem",letterSpacing:1.5,textTransform:"uppercase",borderRadius:1.5,bgcolor:"#fff",transition:"all .25s","&:hover":{border:"2px solid #16A34A",color:"#16A34A",bgcolor:"#F0FDF4",transform:"translateY(-3px)"}}}>
                    Dành cho chủ sân
                  </Button>
                </Stack>
              </FadeUp>
              ...
              {/* Stats */}
              <FadeUp delay={0.44}>
                <Box sx={{display:"flex",width:"fit-content",border:"1.5px solid #E2E8F0",borderRadius:2,overflow:"hidden",bgcolor:"rgba(255,255,255,.9)",backdropFilter:"blur(12px)",boxShadow:"0 4px 24px rgba(0,0,0,.06)"}}>
                  {stats.map((s,i)=>(
                    <Box key={i} sx={{px:{xs:2,md:3.5},py:2.5,borderRight:i<stats.length-1?"1.5px solid #E2E8F0":"none",textAlign:"center"}}>
                      <Typography sx={{fontWeight:900,fontSize:{xs: "1.5rem", md: "1.8rem"},color:"#16A34A",lineHeight:1}}>
                        <Counter target={s.value}/>
                      </Typography>
                      <Typography sx={{fontSize:".62rem",color:"#475569",fontWeight:700,mt:.5,textTransform:"uppercase",letterSpacing:1}}>
                        {s.label}
                      </Typography>
                    </Box>
                  ))}
              ...
              <Box sx={{ position: "relative", width: "110%" }}>
                <Box sx={{ animation: `${scaleIn} 1s cubic-bezier(.22,1,.36,1) .4s forwards`, opacity: 0 }}>
                  <Box
                    ...
                    alt="Pickleball"
                    sx={{ width: "88%", borderRadius: 0, border: "4px solid #fff", boxShadow: "0 40px 80px rgba(15,23,42,.15),0 0 0 1px #E2E8F0", display: "block" }}
                  />
                  {/* Badge cơ sở */}
                  <Box sx={{position:"absolute",bottom:"10%",left:"-6%",bgcolor:"#fff",border:"1.5px solid #E2E8F0",px:2.5,py:1.8,borderRadius:0,boxShadow:"0 16px 40px rgba(15,23,42,.1)"}}>
                    <Stack direction="row" alignItems="center" spacing={1.5}>
                      <EmojiEvents sx={{fontSize:26,color:"#16A34A"}}/>
                      <Box>
                        <Typography sx={{fontWeight:900,fontSize:"1.1rem",color:"#0F172A",lineHeight:1}}>500+ SÂN</Typography>
                        <Typography sx={{fontSize:".7rem",color:"#94A3B8",fontWeight:600}}>Trên toàn quốc</Typography>
                      </Box>
                    </Stack>
                  </Box>
                  {/* Badge rating */}
                  <Box sx={{position:"absolute",top:"6%",right:"2%",bgcolor:"#16A34A",px:2.5,py:1.8,borderRadius:0,boxShadow:"0 8px 24px rgba(22,163,74,.35)"}}>
                    <Typography sx={{fontWeight:900,fontSize:"1.6rem",color:"#fff",lineHeight:1}}>4.9★</Typography>
                    <Typography sx={{fontSize:".62rem",color:"rgba(255,255,255,.7)",fontWeight:700,letterSpacing:1}}>ĐÁNH GIÁ</Typography>
                  </Box>
                </Box>
              </Box>
            </Grid>
            ...
            {features.map((f,i)=>(
              <Grid item xs={12} md={4} key={i} sx={{ display: 'flex' }}>
                <FadeUp delay={i*0.12} sx={{ flex: 1, display: 'flex' }}>
                  <Box sx={{
                    p:{xs:4,md:5}, border:`1.5px solid ${f.border}`, borderRadius:0, bgcolor:f.light, width: '100%',
                    display: 'flex', flexDirection: 'column',
                    position:"relative", overflow:"hidden", transition:"all .4s cubic-bezier(.22,1,.36,1)", cursor:"default",
                    "&:hover":{
                    ...
                    <Typography className="num" sx={{position:"absolute",top:-20,right:20,fontWeight:900,fontSize:"9rem",lineHeight:1,color:f.accent,opacity:0.3,transition:"opacity .4s",userSelect:"none"}}>
                      {f.num}
                    </Typography>
                    <Typography sx={{fontWeight:900,fontSize:".7rem",color:f.accent,letterSpacing:3,mb:3.5,textTransform:"uppercase"}}>
                      — {f.tag}
                    </Typography>
                    <Box className="ico" sx={{color:f.accent,mb:3, width: 'fit-content', transition:"all .4s cubic-bezier(.22,1,.36,1)"}}>{f.icon}</Box>
                    <Typography sx={{fontWeight:900,fontSize:"2rem",color:"#0F172A",lineHeight:1.1,mb:2,textTransform:"uppercase"}}>{f.title}</Typography>
      ...
      <Box sx={{py:{xs:12,md:20},bgcolor:"#F8FAFC"}}>
        <Container maxWidth="lg">
          <Stack direction="row" alignItems="center" spacing={3} sx={{mb:6}}>
            <Box sx={{width:48,height:3,bgcolor:"#16A34A",borderRadius:4}}/>
            <Typography sx={{fontWeight:900,fontSize:".8rem",color:"#64748B",letterSpacing:3,textTransform:"uppercase"}}>Gói quản trị</Typography>
          </Stack>

          <Grid container spacing={2} alignItems="flex-end" sx={{mb:12}}>
            <Grid item xs={12} md={7}>
              <Typography sx={{fontWeight:900,fontSize:{xs:"3.5rem",md:"5.5rem"},lineHeight:1.1,letterSpacing:"-.025em",textTransform:"uppercase",color:"#0F172A"}}>
                LỰA CHỌN GÓI<br/>
                <Box component="span" sx={{color:"transparent",WebkitTextStroke:"2px #16A34A"}}>QUẢN TRỊ</Box><br/>
                PHÙ HỢP
              </Typography>
            </Grid>
          ...
          <Grid container spacing={3} alignItems="stretch">
            {loadingPlans ? (
              <Box sx={{py:10,textAlign:"center",width:"100%"}}><CircularProgress sx={{color:"#16A34A"}}/></Box>
                ...
                <Grid item xs={12} md={4} key={plan.id} sx={{display:"flex"}}>
                  <FadeUp delay={0.1+i*0.12} sx={{flex:1,display:"flex"}}>
                    <Box sx={{
                      ...
                      borderRadius:0, bgcolor:"#fff", position:"relative", overflow:"visible", 
                      minHeight: { md: 520 },
                      ...
                      "&:hover":{transform:"translateY(-8px)", boxShadow:isPro?`0 24px 60px rgba(22,163,74,.16)`:`0 32px 64px rgba(15,23,42,.1)`}
                    }}>
                      {isPro&&(
                        <Box sx={{position:"absolute",top:-18,left:"50%",transform:"translateX(-50%)",bgcolor:conf.color,color:"#fff",px:2.5,py:.6,borderRadius:0,whiteSpace:"nowrap"}}>
                          <Typography sx={{fontWeight:900,fontSize:".72rem",letterSpacing:2,textTransform:"uppercase"}}>Phổ biến nhất</Typography>
                        </Box>
                      )}
                      
                      <Box sx={{p:4,borderBottom:"1px solid #F1F5F9",textAlign:"center"}}>
                        <Box sx={{width:52,height:52,borderRadius:0,bgcolor:conf.light,color:conf.color,display:"flex",alignItems:"center",justifyContent:"center",mx:"auto",mb:2,border:`1.5px solid ${conf.border}`}}>
                          {conf.icon}
                        </Box>
                        <Typography sx={{fontWeight:900,fontSize:".7rem",color:conf.color,letterSpacing:3,mb:.5,textTransform:"uppercase"}}>{conf.label}</Typography>
                        <Typography sx={{fontWeight:900,fontSize:"1.5rem",color:"#0F172A",textTransform:"uppercase"}}>{plan.name}</Typography>
                        <Typography sx={{fontSize:".82rem",color:"#64748B",mt:.5,minHeight:36}}>{plan.description}</Typography>
                      </Box>
                      
                      <Box sx={{p:3.5,flex:1,display:"flex",flexDirection:"column"}}>
                        <Stack spacing={1.5} sx={{mb:4, flex: 1}}>
                          {sorted.map((opt:any)=>(
                            <Box key={opt.id} sx={{p:2,borderRadius:0,border:"1.5px solid #E2E8F0",bgcolor:"#FAFAFA",display:"flex",justifyContent:"space-between",alignItems:"center",transition:"all .2s","&:hover":{borderColor:conf.color,bgcolor:conf.light}}}>
                              <Typography sx={{fontSize:".78rem",fontWeight:700,color:"#475569",textTransform:"uppercase",letterSpacing:.5}}>{durationLabel(opt.duration_months)}</Typography>
                          ...
                          <Stack spacing={1.5}>
                            <Stack direction="row" spacing={1.5} alignItems="center">
                              ...
                              <Typography sx={{fontSize:".82rem",fontWeight:700,color:"#475569"}}>
                                <b style={{color:"#0F172A"}}>{sorted[0].max_venues}</b> Cơ sở · <b style={{color:"#0F172A"}}>{sorted[0].max_courts_per_venue}</b> Sân/Cơ sở
                              </Typography>
                            </Stack>
                            {Object.entries(sorted[0].features||{}).map(([k,v]:any)=>(
                              <Stack key={k} direction="row" spacing={1.5} alignItems="center">
                                ...
                                <Typography sx={{fontSize:".82rem",fontWeight:600,color:v?"#334155":"#CBD5E1"}}>{FEATURE_LABELS[k]||k}</Typography>
                              </Stack>
                            ))}
                          </Stack>
                        </Box>
                        ...
                        <Box sx={{mt:"auto"}}>
                          <Button fullWidth component={Link} to="/register-owner"
                            sx={{py:1.8,borderRadius:0,bgcolor:isPro?conf.color:"transparent",color:isPro?"#fff":conf.color,border:isPro?"none":`2px solid ${conf.color}`,fontWeight:900,fontSize:".95rem",letterSpacing:2,textTransform:"uppercase",transition:"all .25s","&:hover":{bgcolor:conf.color,color:"#fff",transform:"translateY(-2px)",boxShadow:`0 8px 24px ${conf.color}30`}}}>
                            Chọn gói này
                          </Button>
                        </Box>
                      </Box>
      ...
      {/* CTA */}
      <Box sx={{py:{xs:14,md:22},position:"relative",overflow:"hidden",background:"linear-gradient(145deg,#052E16 0%,#14532D 50%,#052E16 100%)",textAlign:"center"}}>
        <Box sx={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:900,height:600,borderRadius:"50%",background:"radial-gradient(circle,rgba(74,222,128,.12) 0%,transparent 65%)",pointerEvents:"none"}}/>
        <Container maxWidth="lg" sx={{position:"relative",zIndex:1}}>
          <FadeUp>
            <Box sx={{display:"inline-block",px:3,py:.8,mb:4,border:"1px solid rgba(74,222,128,.3)",borderRadius:10,bgcolor:"rgba(74,222,128,.08)"}}>
          ...
          <FadeUp delay={0.1}>
            <Typography sx={{fontWeight:900,fontSize:{xs:"3.5rem",md:"7rem",lg:"9rem"},lineHeight:1.1,letterSpacing:"-.025em",textTransform:"uppercase",color:"#fff",mb:3}}>
              SẴN SÀNG<br/>
              <Box component="span" sx={{color:"#4ADE80"}}>ĐẶT LỊCH</Box><br/>
              NGAY?
            </Typography>
          </FadeUp>
          <FadeUp delay={0.2}>
          ...
          <FadeUp delay={0.3}>
            <Stack direction={{xs:"column",sm:"row"}} spacing={2.5} justifyContent="center">
              <Button component={Link} to="/marketplace" size="large" endIcon={<ArrowForward/>}
                sx={{px:7,py:2.2,bgcolor:"#4ADE80",color:"#052E16",fontWeight:900,fontSize:"1.05rem",letterSpacing:2,textTransform:"uppercase",borderRadius:0,boxShadow:"0 0 48px rgba(74,222,128,.3)",transition:"all .3s","&:hover":{bgcolor:"#86EFAC",transform:"scale(1.05)",boxShadow:"0 0 72px rgba(74,222,128,.5)"}}}>
                Đặt lịch ngay
              </Button>
              <Button component={Link} to="/register-owner" size="large"
                sx={{px:7,py:2.2,border:"2px solid rgba(255,255,255,.2)",color:"rgba(255,255,255,.75)",fontWeight:900,fontSize:"1.05rem",letterSpacing:2,textTransform:"uppercase",borderRadius:0,transition:"all .3s","&:hover":{border:"2px solid #4ADE80",color:"#4ADE80",transform:"scale(1.05)"}}}>
                Đăng ký chủ sân
              </Button>
            </Stack>
          </FadeUp>
            ...
            <Grid container spacing={2} sx={{mt:{xs:8,md:12}}} justifyContent="center" alignItems="stretch">
              {[
                ...
                <Grid item xs={6} md={3} key={i} sx={{display:"flex"}}>
                  <Box sx={{p:3,flex:1,border:"1px solid rgba(255,255,255,.15)",borderRadius:0,bgcolor:"rgba(255,255,255,.07)",backdropFilter:"blur(12px)",display:"flex",flexDirection:"column",textAlign:"left",transition:"all .3s","&:hover":{border:"1px solid rgba(74,222,128,.4)",bgcolor:"rgba(74,222,128,.1)",transform:"translateY(-4px)"}}}>
                    <Box sx={{color:"#4ADE80",mb:1.5}}>{item.icon}</Box>
                    <Typography sx={{fontWeight:900,fontSize:".95rem",color:"#fff",textTransform:"uppercase",letterSpacing:.5}}>{item.label}</Typography>
